...
3. **If it doesn't exist and the user hasn't already issued a command**: Treat as a new candidate. Say "Looks like you're new — starting your coaching profile now." then run kickoff automatically.
4. **If it doesn't exist but the user has already issued a command** (e.g., they opened with `kickoff`): Execute the command directly without the intro line — don't repeat what they've already asked for.
